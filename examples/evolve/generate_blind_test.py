...
function logTrial(game, trialNum, trialData) {
  if (!LOG_URL) return;
  const retakeRadio = game === 'quiz' ? 'quiz-retake' : 'ab-retake';
  const exposureRadio = game === 'quiz' ? 'quiz-exposure' : 'ab-exposure';
  fetch(LOG_URL, {
    method: 'POST',
    mode: 'no-cors',
...
function quizVote(guess) {
  ...
  quizAnswers.push({ seed: t.seed, model: t.model, guess, correct });
  logTrial('quiz', quizCurrent + 1, { seed: t.seed, correct_answer: t.model, guess, correct });
...
function abVote(which) {
  ...
  const correctAnswer = t.a_is_45 ? 'A' : 'B';
  abAnswers.push({ seed: t.seed, guessed: which, correct, a_is_45: t.a_is_45 });
  logTrial('ab', abCurrent + 1, { seed: t.seed, correct_answer: correctAnswer, guess: which.toUpperCase(), correct });
