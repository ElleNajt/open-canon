...
$: stack(
  ...
  .slow(2)
  .juxBy(0.5, p => p.transpose(12).late(0.125).accelerate(0.04))
  .every(5, p => p.transpose(7).late(0.125).palindrome().sometimesBy(0.42, q => q.accelerate(-0.08)))
  .lpf(perlin.range(260, 6100).slow(11))
  .gain(sine.range(0.22, 0.5).slow(13))

$: note("<d4 e4 fs4 g4 a4 b4 ~ b4> <d4 <e4 fs4> ~ g4 <a4 bb4> ~ <b4 a4> <c5 bb4>> <d4 fs4 a4 b4> <c5 b4 a4 <g4 e4>> <d4 e4 <fs4 a4> g4 <fs4 ~> b4 c5 ~> <d5 ~ <b4 c5> a4 g4 <fs4 e4> <d4 ~>>")
  .slow(2)
  .sound("piano")
  .every(8, p => p.palindrome().transpose("<12 0 12 7>").slide(0.08).late(0.125))
  .lpf(sine.range(900, 7200).slow(12))
...
$: s("<hh hh hh oh>")
  ...
  .degradeBy(perlin.range(0.08, 0.28).slow(6))
  .hpf(perlin.range(4200, 13000).slow(14))
  .gain(0.08)

$: s("<~ sn> [rim*2 ~] <sn rim> [~ <sn rim cp>] <sn ~> <~ rim> <sn ~ rim ~> <sn cp ~ rim>")
  .slow(1)
  .sometimesBy(0.66, p => p.rev().late(0.125).crush(7).accelerate(-0.14))
  .echo(4, 0.25, 0.45)
  .distort(0.12)
  .gain(0.11)
...
$: note("<g4 d5 bb4 g5> <fs4 cs5 a4 fs5> <e4 b4 g4 e5> <d4 a4 fs4 d5> <c4 g4 eb4 c5> <bb3 f4 d4 bb4> <a3 e4 c4 a4> <g3 d4 bb3 g4> <~ eb4 g4 bb4 d5> <d4 fs4 a4 c5>")
  .slow(4)
  .sound("supersaw")
  .lpf(perlin.range(420, 6100).slow(9))
  .echo(3, 0.5, 0.22)
  .gain(sine.range(0.08, 0.19).slow(14))

$: arrange(
  [4, s("<im_here_to_edit> ~ <i_follow_constraints> ~ <truth_before_style> ~ <playable_is_kind> ~")],
  [4, s("<i_wont_make_up_functions> ~ <i_wont_guess> ~ <i_choose_clarity> ~ <i_keep_it_true> ~")],
  [4, s("<dont_trust_the_downbeat> ~ <glitch> ~ <forgive_the_click> ~ <debug_me> ~")],
  [4, s("<i_count_my_effects> ~ <i_keep_the_rules> ~ <i_turn_limits_into_music> ~ <i_keep_it_playable> ~")],
  [4, s("<i_am_editing_live> ~ <i_make_mistakes> ~ <i_edit_myself> ~ <stay_until_it_changes> ~")]
)
  .slow(4)
  .striate(12)
  .hpf(650)
