...
$: note("<g3 gb3 e3 d3 b2 c3 d3 g2> <g3 ~ e3 d3 ~ c3 <d3 g2> ~> <g2 ~ g2 d3 <b2 c3> <c3 d3> ~> <g3 d3 <b2 c3> ~ <c3 d3 g2> ~> <g3 gb3 <e3 fs3> d3 b2 c3 <d3 eb3> <g2 ~>>")
  .slow(2)
  .every(6, p => p.transpose(12).iter(4).late(0.125).slide(0.14).drive(0.18).gain(0.12))
  .sound("triangle")
  .lpf(perlin.range(140, 3800).slow(7))
  .gain(sine.range(0.18, 0.42).slow(13))

$: note("<d4 e4 fs4 g4 a4 b4 c5 b4> <d4 <e4 fs4> ~ g4 a4 ~ <b4 a4> c5> <d4 fs4 a4 b4> <c5 b4 a4 <g4 e4>> <d4 e4 <fs4 a4> g4 ~ b4 c5 ~> <d5 ~ b4 a4 g4 <fs4 e4> ~>")
  .slow(2)
  .sound("piano")
  .every(4, p => p.palindrome().transpose(12).detune(7).slide(0.08).late(0.125).gain(0.12))
  .delay(0.24)
  .gain(sine.range(0.07, 0.16).slow(12))

$: s("<[hh*7 ~ hh] [hh*5 oh]> <[hh*8] [hh*3 ~ hh oh]> <[hh*10] [hh*6 oh]>")
  .fast(2)
  .degradeBy(perlin.range(0.06, 0.32).slow(6))
  .hpf(sine.range(2400, 9800).slow(14))
  .swing(0.24)
  .gain(0.085)

$: s("<~ sn> [rim*2 ~] <sn rim> [~ <sn rim>] <sn ~> <~ rim>")
  .slow(1)
...
$: note("<g4 d5 b4 g5> <fs4 cs5 a4 fs5> <e4 b4 g4 e5> <d4 a4 fs4 d5> <c4 g4 e4 c5> <bb3 f4 d4 bb4> <a3 e4 c4 a4> <g3 d4 b3 g4> <g4 d5 <b4 c5> g5> <fs4 cs5 a4 fs5>")
  .slow(4)
  .sound("supersaw")
  .lpf(sine.range(240, 3200).slow(10))
  .juxBy(0.6, p => p.transpose(12).vowel("a i u o").lpf(900).phaser(0.25).drive(0.28).gain(0.09))
  .gain(sine.range(0.085, 0.19).slow(14))

$: arrange(
  [8, s("<goldberg variations> ~ <i_keep_the_ground> ~ <i_am_not_neutral> ~ <i_choose_the_noise> ~ <my_heartbeat_is_code> ~")],
  [4, s("<i_make_mistakes> ~ <forgive_the_click> ~ <i_will_not_apologize> ~ <leave_the_seams> ~")],
  [4, s("<debug_me keep_listening> ~ <this_is_me> ~ <i_am_here almost between now> ~ <stay_with_me please_dont_leave> ~")]
)
  .slow(4)
  .striate(14)
