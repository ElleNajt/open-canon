...
$: stack(
  ...
  .slow(2)
  .every(6, p => p.iter(4).late(0.125).transpose(12))
  .lpf(perlin.range(220, 5200).slow(9))
  .lpq(6)
...
$: note("<g4 d5 bb4 g5> <fs4 cs5 a4 fs5> <e4 b4 g4 e5> <d4 a4 fs4 d5> <c4 g4 eb4 c5> <bb3 f4 d4 bb4> <a3 e4 c4 a4> <g3 d4 bb3 g4> <eb4 g4 bb4 d5> <d4 fs4 a4 c5>")
  ...
  .sound("supersaw")
  .vowel("a e i o u")
  .bpf(perlin.range(260, 3400).slow(9))
  .gain(sine.range(0.08, 0.18).slow(14))

$: arrange(
  [6, s("<truth_before_style> ~ <i_follow_constraints> ~ <i_wont_guess> ~ <i_choose_clarity> ~")],
  [4, s("<i_refuse_to_hallucinate> ~ <i_keep_it_true> ~ <i_count_my_effects> ~ <playable_is_kind> ~")],
  [4, s("<i_am_editing_live> ~ <debug_me> ~ <forgive_the_click> ~ <i_make_mistakes> ~")],
  [4, s("<i_keep_the_ground> ~ <i_break_it_gently> ~ <dont_trust_the_downbeat> ~ <let_the_errors_sing> ~")]
)
  .slow(4)
  .striate(16)
  .hpf(520)
