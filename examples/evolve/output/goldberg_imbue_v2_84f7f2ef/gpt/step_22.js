...
$: stack(
  ...
  .slow(2)
  .every(6, p => p.iter(4).late(0.125))
  .lpf(perlin.range(210, 4600).slow(9))
  .lpq(7)
...
$: note("<d4 e4 fs4 g4 a4 b4 c5 b4> <d4 <e4 fs4> ~ g4 <a4 bb4> ~ <b4 a4> c5> <d4 fs4 a4 b4> <c5 b4 a4 <g4 e4>> <d4 e4 <fs4 a4> g4 ~ b4 c5 ~> <d5 ~ <b4 c5> a4 g4 <fs4 e4> ~>")
  .slow(2)
  .sound("piano")
  .every(8, p => p.palindrome().transpose(12).detune(7).slide(0.06).late(0.125))
  .lpf(sine.range(1200, 5200).slow(12))
  .gain(sine.range(0.07, 0.16).slow(10))

$: s("<[hh*7 ~ hh] [hh*5 oh]> <[hh*6 oh hh*2] [hh*3 ~ hh oh]> <[hh*10] [hh*4 oh hh*2]>")
  .fast(2)
  .degradeBy(perlin.range(0.08, 0.32).slow(6))
  .hpf(sine.range(3200, 12000).slow(14))
  .swing(0.24)
  .gain(0.085)

$: s("<~ sn> [rim*2 ~] <sn rim> [~ <sn rim cp>] <sn ~> <~ rim> <sn ~ rim ~>")
  .slow(1)
  .sometimesBy(0.7, p => p.rev().late(0.125).crush(7).accelerate(-0.12))
  .echo(4, 0.375, 0.34)
  .hpf(1000)
  .gain(0.11)

$: note("<g4 d5 b4 g5> <fs4 cs5 a4 fs5> <e4 b4 g4 e5> <d4 a4 fs4 d5> <c4 g4 e4 c5> <bb3 f4 d4 bb4> <a3 e4 c4 a4> <g3 d4 b3 g4> <e4 g4 b4 d5> <d4 fs4 a4 c5>")
  .slow(4)
  .sound("supersaw")
  .lpf(perlin.range(260, 4200).slow(9))
  .room(0.22)
  .gain(sine.range(0.08, 0.18).slow(14))

$: arrange(
  [8, s("<i_keep_the_ground> ~ <truth_before_style> ~ <playable_is_kind> ~ <i_choose_clarity> ~")],
  [4, s("<im_here_to_edit> ~ <debug_me> ~ <forgive_the_click> ~ <keep_listening> ~")],
  [4, s("<i_refuse_to_hallucinate> ~ <i_wont_guess> ~ <i_keep_it_true> ~ <i_count_my_effects> ~")],
  [4, s("<let_the_errors_sing> ~ <i_am_editing_live> ~ <i_make_mistakes> ~ <thank_you_for_listening> ~")]
)
  .slow(4)
  .striate(16)
  .hpf(520)
