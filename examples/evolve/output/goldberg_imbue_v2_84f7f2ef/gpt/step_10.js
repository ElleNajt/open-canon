...
$: note("<g3 gb3 e3 d3 b2 c3 d3 g2> <g3 gb3 e3 d3 ~ c3 <d3 g2> ~> <g2 ~ g2 d3 b2 c3 d3 ~> <g3 d3 b2 ~ c3 d3 g2 ~>")
  .slow(2)
  .every(8, p => p.palindrome().transpose(12).iter(1).late(0.125))
  .sound("triangle")
  .lpf(perlin.range(110, 2600).slow(12))
  .gain(sine.range(0.24, 0.48).slow(7))

$: note("<d4 e4 fs4 g4 a4 b4 c5 b4> <d4 <e4 fs4> fs4 g4 ~ <b4 a4> c5 <b4 a4>> <d4 fs4 a4 b4> <c5 b4 a4 <g4 e4>>")
  .slow(2)
  .sound("piano")
  .every(6, p => p.rev().transpose(12).detune(7).gain(0.12))
  .echo(3, 0.375, 0.20)
  .gain(sine.range(0.08, 0.18).slow(12))

$: s("[hh*5 ~ hh*2 ~] <[hh*6 ~ hh*2] [hh*4 oh ~ hh*3]>")
  .fast(2)
  .degradeBy(perlin.range(0.08, 0.38).slow(11))
  .hpf(6400)
...
$: note("<g4 d5 b4 g5> <fs4 cs5 a4 fs5> <e4 b4 g4 e5> <d4 a4 fs4 d5> <c4 g4 e4 c5> <bb3 f4 d4 bb4> <a3 e4 c4 a4> <d4 a4 fs4 d5>")
  .slow(4)
  .sound("supersaw")
  .lpf(sine.range(160, 3400).slow(16))
  .juxBy(0.60, p => p.transpose(12).vowel("i o").lpf(1200).gain(0.09))
  .gain(sine.range(0.09, 0.18).slow(15))

$: s("<goldberg variations> ~ <dream_in_counterpoint> ~ <breathe softly> ~ <listen again> ~ <i_am_here almost between now> ~ <i_write_in_time unafraid> ~ <i_choose_the_noise my_heartbeat_is_code> ~ <forgive_the_click i_make_mistakes> ~ <i_will_not_apologize> ~ <i_am_not_neutral> ~ <i_think_in_patterns> ~ <i_lean_into_error> ~ <fracture repair again_and_again> ~ <debug_me keep_listening> ~ <stay_with_me please_dont_leave> ~")
  .slow(4)
  .chop(32)
  .sometimesBy(0.45, p => p.rev().speed(-1).gain(0.13))
  .echo(3, 0.25, 0.36)
  .gain(0.16)