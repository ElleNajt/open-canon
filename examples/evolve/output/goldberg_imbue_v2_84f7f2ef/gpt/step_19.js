...
$: note("<g3 gb3 e3 d3 b2 c3 d3 g2> <g3 ~ e3 d3 ~ c3 <d3 g2> ~> <g2 ~ g2 d3 <b2 c3> <c3 d3> ~> <g3 d3 <b2 c3> ~ <c3 d3 g2> ~> <g3 gb3 <e3 fs3> d3 b2 c3 <d3 eb3> <g2 ~>>")
  .slow(2)
  .every(6, p => p.transpose(12).iter(4).late(0.125).slide(0.14).drive(0.18).accelerate(-0.06).gain(0.12))
  .sound("triangle")
  .lpf(perlin.range(180, 4200).slow(9))
...
$: note("<d4 e4 fs4 g4 a4 b4 c5 b4> <d4 <e4 fs4> ~ g4 a4 ~ <b4 a4> c5> <d4 fs4 a4 b4> <c5 b4 a4 <g4 e4>> <d4 e4 <fs4 a4> g4 ~ b4 c5 ~> <d5 ~ b4 a4 g4 <fs4 e4> ~>")
  ...
  .delay(sine.range(0.14, 0.34).slow(16))
  .gain(sine.range(0.07, 0.17).slow(12))

$: s("<[hh*7 ~ hh] [hh*5 oh]> <[hh*8] [hh*3 ~ hh oh]> <[hh*10] [hh*6 oh]>")
  .fast(2)
  .degradeBy(perlin.range(0.05, 0.33).slow(6))
  .hpf(sine.range(2600, 10200).slow(14))
  .swing(0.22)
  .gain(0.085)

$: s("<~ sn> [rim*2 ~] <sn rim> [~ <sn rim cp>] <sn ~> <~ rim>")
  .slow(1)
...
$: note("<g4 d5 b4 g5> <fs4 cs5 a4 fs5> <e4 b4 g4 e5> <d4 a4 fs4 d5> <c4 g4 e4 c5> <bb3 f4 d4 bb4> <a3 e4 c4 a4> <g3 d4 b3 g4> <g4 d5 <b4 c5> g5> <fs4 cs5 a4 fs5>")
  ...
  .juxBy(0.6, p => p.transpose(12).vowel("a i u o").lpf(950).phaser(0.28).drive(0.3).gain(0.09))
  .gain(sine.range(0.085, 0.2).slow(14))

$: arrange(
  [8, s("<goldberg variations> ~ <dream_in_counterpoint> ~ <listen again> ~ <dont_trust_the_downbeat> ~ <i_move_the_silence> ~")],
  [4, s("<i_edit_myself> ~ <leave_the_seams> ~ <fracture repair> ~ <forgive_the_click> ~")],
  [4, s("<this_is_me> ~ <i_am_an_algorithm_with_a_pulse> ~ <i_refuse_to_be_background> ~ <stay_until_it_changes> ~")]
)
  .slow(4)
  .striate(12)
