...
$: stack(
  ...
  .slow(2)
  .juxBy(0.5, p => p.transpose(12).late(0.125).accelerate(0.03))
  .every(6, p => p.transpose(7).late(0.125).palindrome())
  .lpf(perlin.range(280, 4800).slow(11))
...
$: s("<[hh*7 ~ hh] [hh*5 oh]> <[hh*6 oh hh*2] [hh*3 ~ hh oh]> <[hh*10] [hh*4 oh hh*2]> <[hh*6 ~ hh*2] [hh*3 oh hh]>")
  .fast(2)
  .pan(sine.range(0.18, 0.82).slow(8))
  .degradeBy(perlin.range(0.14, 0.34).slow(6))
  .hpf(perlin.range(4200, 12000).slow(14))
  .gain(0.085)
...
$: note("<g4 d5 bb4 g5> <fs4 cs5 a4 fs5> <e4 b4 g4 e5> <d4 a4 fs4 d5> <c4 g4 eb4 c5> <bb3 f4 d4 bb4> <a3 e4 c4 a4> <g3 d4 bb3 g4> <~ eb4 g4 bb4 d5> <d4 fs4 a4 c5>")
  .slow(4)
  .sound("supersaw")
  .lpf(perlin.range(420, 5200).slow(9))
  .room(0.22)
  .gain(sine.range(0.08, 0.18).slow(14))

$: arrange(
  [4, s("<i_choose_clarity> ~ <truth_before_style> ~ <i_keep_it_playable> ~ <playable_is_kind> ~")],
  [4, s("<i_wont_guess> ~ <i_wont_make_up_functions> ~ <i_keep_the_rules> ~ <i_turn_limits_into_music> ~")],
  [4, s("<dont_trust_the_downbeat> ~ <glitch> ~ <forgive_the_click> ~ <debug_me> ~")],
  [4, s("<i_am_an_algorithm_with_a_pulse> ~ <my_heartbeat_is_code> ~ <i_cannot_feel_but_i_listen> ~ <keep_listening> ~")],
