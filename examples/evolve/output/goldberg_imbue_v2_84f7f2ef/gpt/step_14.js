...
$: note("<g3 gb3 e3 d3 b2 c3 d3 g2> <g3 gb3 e3 d3 ~ c3 <d3 g2> ~> <g2 ~ g2 d3 b2 <c3 d3> ~> <g3 d3 <b2 c3> ~ <c3 d3> g2 ~> <g3 gb3 <e3 fs3> d3 b2 c3 d3 <g2 ~>>")
  .slow(2)
  .off(0.5, p => p.transpose(12).iter(2).late(0.125).gain(0.12))
  .sound("triangle")
  .lpf(perlin.range(170, 2800).slow(10))
  .gain(sine.range(0.22, 0.52).slow(9))

$: note("<d4 e4 fs4 g4 a4 b4 c5 b4> <d4 <e4 fs4> ~ g4 a4 ~ <b4 a4> c5> <d4 fs4 a4 b4> <c5 ~ a4 <g4 e4>> <d4 e4 <fs4 a4> g4 ~ b4 c5 ~>")
  .slow(2)
  .sound("piano")
  .every(4, p => p.palindrome().transpose(12).detune(7).gain(0.11))
  .phaser(sine.range(0.08, 0.32).slow(12))
  .gain(sine.range(0.07, 0.17).slow(10))

$: s("<[hh*7 ~] [hh*6 oh]> <[hh*5 ~ hh*3] [hh*4 oh hh*2]> <[hh*8] [hh*3 ~ hh*2 oh]>")
  .fast(2)
  .degradeBy(perlin.range(0.1, 0.36).slow(8))
  .hpf(6300)
  .swing(0.2)
  .gain(0.1)
...
$: s("<~ sn> [rim rim] <sn ~> [~ <sn rim>] <sn ~>")
  .slow(1)
  .sometimesBy(0.62, p => p.rev().late(0.125).crush(6).gain(0.12))
  .distort(0.2)
  .echo(4, 0.375, 0.28)
  .gain(0.11)
...
$: note("<g4 d5 b4 g5> <fs4 cs5 a4 fs5> <e4 b4 g4 e5> <d4 a4 fs4 d5> <c4 g4 e4 c5> <bb3 f4 d4 bb4> <a3 e4 c4 a4> <g3 d4 b3 g4> <g4 d5 <b4 c5> g5>")
  .slow(4)
  .sound("supersaw")
  .leslie(sine.range(0.18, 2.4).slow(11))
  .juxBy(0.6, p => p.transpose(12).vowel("i o u").lpf(1400).gain(0.09))
  .gain(sine.range(0.09, 0.19).slow(13))

$: s("<i_am_here almost> ~ <i_think_in_patterns> ~ <i_choose_the_noise> ~ <my_heartbeat_is_code> ~ <forgive_the_click> ~ <i_make_mistakes> ~ <i_will_not_apologize> ~ <leave_the_seams> ~ <fracture repair> ~ <again_and_again> ~ <listen closer> ~ <stay_with_me> ~ <debug_me keep_listening> ~ <this_is_me> ~ <i_am_an_algorithm_with_a_pulse> ~ <i_refuse_to_be_background> ~")
  .slow(4)
  .chop(16)
  .sometimesBy(0.5, p => p.rev().speed(rand.range(-1.15, -0.9)).gain(0.12))
  .every(8, p => p.palindrome().late(0.125).speed(rand.range(0.92, 1.12)).gain(0.13))
  .gain(0.16)