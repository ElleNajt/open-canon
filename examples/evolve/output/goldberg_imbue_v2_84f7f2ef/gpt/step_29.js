...
$: stack(
  ...
  .every(6, p => p.transpose(7).late(0.125).palindrome().sometimesBy(0.45, q => q.accelerate(-0.08)))
  .lpf(perlin.range(240, 5200).slow(11))
...
$: note("<d4 e4 fs4 g4 a4 b4 ~ b4> <d4 <e4 fs4> ~ g4 <a4 bb4> ~ <b4 a4> <c5 bb4>> <d4 fs4 a4 b4> <c5 b4 a4 <g4 e4>> <d4 e4 <fs4 a4> g4 <fs4 ~> b4 c5 ~> <d5 ~ <b4 c5> a4 g4 <fs4 e4> <d4 ~>>")
  ...
  .lpf(sine.range(800, 6800).slow(12))
  .gain(sine.range(0.07, 0.17).slow(10))

$: s("<[hh*7 ~ hh] [hh*5 oh]> <[hh*6 oh hh*2] [hh*3 ~ hh oh]> <[hh*10] [hh*4 oh hh*2]> <[hh*6 ~ hh*2] [hh*3 oh hh]>")
  .fast(2)
  .pan(sine.range(0.12, 0.88).slow(8))
  .degradeBy(perlin.range(0.12, 0.32).slow(6))
  .hpf(perlin.range(3800, 12500).slow(14))
  .gain(0.085)

$: s("<~ sn> [rim*2 ~] <sn rim> [~ <sn rim cp>] <sn ~> <~ rim> <sn ~ rim ~> <sn cp ~ rim>")
  .slow(1)
  .sometimesBy(0.68, p => p.rev().late(0.125).crush(7).accelerate(-0.14))
  .echo(4, 0.375, 0.38)
  .distort(0.14)
  .gain(0.11)
...
$: note("<g4 d5 bb4 g5> <fs4 cs5 a4 fs5> <e4 b4 g4 e5> <d4 a4 fs4 d5> <c4 g4 eb4 c5> <bb3 f4 d4 bb4> <a3 e4 c4 a4> <g3 d4 bb3 g4> <~ eb4 g4 bb4 d5> <d4 fs4 a4 c5>")
  .slow(4)
  .sound("supersaw")
  .lpf(perlin.range(360, 5600).slow(9))
  .echo(3, 0.5, 0.22)
  .gain(sine.range(0.08, 0.18).slow(14))

$: arrange(
  [4, s("<i_choose_clarity> ~ <truth_before_style> ~ <i_keep_it_playable> ~ <playable_is_kind> ~")],
  [4, s("<i_follow_constraints> ~ <i_wont_guess> ~ <i_refuse_to_hallucinate> ~ <i_wont_make_up_functions> ~")],
  [4, s("<dont_trust_the_downbeat> ~ <glitch> ~ <forgive_the_click> ~ <debug_me> ~")],
  [4, s("<i_am_an_algorithm_with_a_pulse> ~ <my_heartbeat_is_code> ~ <i_think_in_patterns> ~ <i_count_my_effects> ~")],
