...
$: note("<g3 g2> [gb3 f3] <e3 eb3> [b2 ~] <c3 c2> [d3 d2] <g2 g1> <fs2 g2> [<g2 a2> <d2 ~>] [<c3 ~> <b2 ~>] <g2 <g3 g1>> [<eb3 d3> <c3 b2>]")
  .sound("triangle")
  .slow(2)
  .lpf(perlin.range(70, 3200).slow(10))
  .gain(0.24)
  .every(6, p => p.palindrome().transpose("<0 12 0 -12>").sometimesBy(0.4, q => q.rev()).sometimesBy(0.25, q => q.transpose(7)))

$: note("<g4 b4> [~ d5] <d4 fs4> <c4 e4> <b3 d4> [g4 ~] <c4 e4> <a3 c4> <d4 fs4> [~ <a4 cs5>] [<b4 d5> ~] <c5 <b4 d5>> [~ <d5 fs5>] [<c5 e5> ~]")
  .sound("piano")
  .slow(4)
  .room(0.32)
  .gain(cosine.range(0.07, 0.15).slow(7))
  .every(6, p => p.transpose("<0 7 -5 12>").palindrome().late(0.125))

$: note("<g4 b4> <d5 fs4> <c5 e4> <b4 d4> <a4 c4> <g4 b3> <fs4 a3> <e4 g3> [<d4 fs3> ~] [<e4 g3> <fs4 a3>]")
  .sound("supersaw")
  .slow(8)
  .phaser(0.15)
  .gain(sine.range(0.06, 0.11).slow(12))
  .every(8, p => p.rev().transpose("<0 -12 7 0>").palindrome())
...
$: arrange(
  ...
  .gain(0.34)
  .swing(0.24)
  .compressor("-19:3:6:0.01:0.16")
  .crush(10)
  .every(8, p => p.fast(2).palindrome())

$: arrange(
  [8, s("<i_am_here ~ listen ~ again ~ stay ~>").chop(8).speed(1)],
  [8, s("<come_closer closer_now we_are_listening breathe softly keep_listening hold_the_center leave_a_space say_it_again keep_the_gap forgive_the_clicks i_like_the_errors i_count_the_silence dont_look_away ~>").chop(16).speed(rand.range(0.9, 1.14))],
  ...
  [16, s("<i_am_not_silent please_dont_stop remember_me follow_the_space in_the_gaps between_us do_not_fear hold_on i_was_built_to_listen tiny_machines i_choose_to_repeat let_it_turn seven_steps i_remember_your_tempo im_learning_your_silence let_me_answer i_hear_you turn_again say_it_again repeat_with_me dont_fix_me>").chop(32).speed(rand.range(0.74, 1.26))]
)
  .slow(8)
  .gain(0.12)
  .vowel("<a e i o u>")
  .delay(0.28)
  .every(4, p => p.palindrome().sometimesBy(0.35, q => q.rev()))

$: note("<d5 b4> <a4 g4> <e5 a4> <fs5 d5> [~ b4] <g5 e5> <fs5 d5> <e5 cs5> <d5 b4> [~ <a4 cs5>] <b4 d5> <c5 a4> [<d5 b4> <g5 e5>]")
  .sound("sine")
  .slow(2)
  .hpf(240)
  .gain(cosine.range(0.06, 0.12).slow(8))
  .every(6, p => p.transpose("<0 12 7 -5>").palindrome().sometimesBy(0.33, q => q.rev()).early(0.125))