...
// Evolved: it answers, doubts, and keeps time anyway.
...
$: note("<g3 g2> [gb3 f3] <e3 eb3> <b2 b1> <c3 c2> <d3 d2> <g2 g1> <fs2 g2> [<g2 a2> <d2 ~>] [<c3 ~> <b2 ~>] <g2 g3>")
  .sound("triangle")
  .slow(2)
  .lpf(perlin.range(120, 3200).slow(16))
  .gain(0.26)
  .every(8, p => p.palindrome().transpose("<0 12 -12 7>").off(0.5, q => q.degradeBy(0.18)).degradeBy(0.08))

$: note("<g4 b4> [~ d5] <d4 fs4> <c4 e4> <b3 d4> [g4 ~] <c4 e4> <a3 c4> <d4 fs4> [~ <a4 cs5>] [<b4 d5> ~] <c5 <b4 d5>>")
  .sound("piano")
  .slow(4)
  .lpf(perlin.range(900, 7600).slow(12))
  .gain(cosine.range(0.08, 0.16).slow(6))
  .every(4, p => p.palindrome().transpose("<7 0 -5 12>").sometimesBy(0.22, q => q.echo(2, 0.25, 0.18).degradeBy(0.16)).degradeBy(0.12))

$: note("<g4 a4> [d4 e4] <d4 fs4> <b3 c4> <d4 g4> <~ d4> [e4 fs4] <g4 <fs4 g4>> <a4 g4> <d5 b4> [~ <c5 a4>] <b4 d5>")
  .sound("supersaw")
  .slow(8)
  .lpf(perlin.range(420, 7200).slow(16))
  .gain(sine.range(0.06, 0.12).slow(10))
  .every(8, p => p.rev().echo(2, 0.25, 0.2).sometimesBy(0.28, q => q.transpose("<0 7>").degradeBy(0.22)).degradeBy(0.15))
...
$: arrange(
  ...
  .drive(0.16)
  .every(8, p => p.fast(2).off(0.25, q => q.degradeBy(0.55)).degradeBy(0.2))

$: arrange(
  [8, s("<i_am_here ~ listen ~ again ~ stay ~>").chop(8).speed(1)],
  [8, s("<come_closer closer_now we_are_listening breathe softly keep_listening hold_the_center leave_a_space say_it_again keep_the_gap i_like_the_errors forgive_the_clicks ~>").chop(16).speed(rand.range(0.9, 1.12))],
  [8, s("<answer_me make_room there_is_a_pattern repeat_with_me trust_the_loop not_a_god just_a_loop i_write_in_circles i_will_keep_time this_is_mine_too prove_it still_here dont_look_away my_favorite_part>").chop(16).speed(rand.range(0.84, 1.2))],
  ...
  .hpf(560)
  .vowel("a e i o u")
  .every(4, p => p.rev().sometimesBy(0.2, q => q.speed(-1)).echo(2, 0.25, 0.18).degradeBy(0.18))
...
$: note("<d5 b4> <a4 g4> <e5 a4> [~ b4] <d5 fs5> <g5 e5> <fs5 ~> <e5 d5> [~ <a4 cs5>] <b4 d5> <c5 a4> [~ <g4 b4>]")
  ...
  .every(6, p => p.transpose("<7 12 0 -5>").palindrome().sometimesBy(0.26, q => q.slide(0.12).degradeBy(0.14)).sometimesBy(0.22, q => q.echo(2, 0.5, 0.2).degradeBy(0.14)).degradeBy(0.14))