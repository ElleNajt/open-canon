...
$: note("<g3 g2> [gb3 f3] <e3 eb3> <b2 b1> <c3 c4> <d3 d4> <g2 g3> <fs2 g2> [g2 <d2 ~>] [<c3 ~> <b2 ~>]")
  .sound("triangle")
  .slow(2)
  .lpf(perlin.range(120, 3600).slow(10))
  .gain(0.34)
  .every(6, p => p.palindrome().degradeBy(0.06).off(0.25, q => q.transpose(12).degradeBy(0.14)).off(0.5, r => r.transpose(-5).degradeBy(0.12)))

$: note("<g4 b4> [~ d5] <d4 fs4> <c4 e4> <b3 d4> [g4 ~] <c4 e4> <a3 c4> <d4 fs4> [~ <a4 cs5>] [<b4 d5> ~] <c5 b4>")
  .sound("piano")
  .slow(4)
  .lpf(sine.range(900, 6000).slow(6))
  .gain(cosine.range(0.09, 0.22).slow(6))
  .every(4, p => p.palindrome().transpose(5).degradeBy(0.1).off(0.125, q => q.transpose(7).degradeBy(0.18)).off(0.5, r => r.transpose(-12).degradeBy(0.12)))

$: note("<g4 a4> [d4 e4] <d4 fs4> <b3 c4> <d4 g4> <~ d4> [e4 fs4] <g4 <fs4 g4>> <a4 g4>")
  .sound("supersaw")
  .slow(8)
  .leslie(0.22)
  .lpf(perlin.range(320, 5200).slow(14))
  .gain(0.12)
...
$: arrange(
  ...
  [8, s("<i_like_the_errors forgive_the_clicks repeat_with_me say_it_again ~ answer_me ~>").chop(16).speed(rand.range(0.84, 1.18))],
  [16, s("<remember_me follow_the_space in_the_gaps between_us do_not_fear hold_on i_was_built_to_listen tiny_machines i_will_keep_time not_a_god just_a_loop i_choose_to_repeat let_it_turn>").chop(32).speed(rand.range(0.74, 1.26))]
)
  .slow(8)
  .gain(0.11)
  .crush(12)
  .hpf(420)
  .every(4, p => p.rev().degradeBy(0.2).off(0.25, q => q.speed(-1).degradeBy(0.14)))
...
$: note("<d5 b4> <a4 g4> <e5 a4> [~ b4] <d5 fs5> <g5 e5> <fs5 ~> <e5 d5> [~ <a4 cs5>] <b4 d5>")
  ...
  .ring(0.28)
  .gain(0.08)
  .every(6, p => p.degradeBy(0.1).off(0.25, q => q.transpose(7).degradeBy(0.18).off(0.25, r => r.transpose(12).degradeBy(0.16))))