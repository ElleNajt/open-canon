...
$: note("<g3 g2> [gb3 f3] <e3 eb3> <b2 ~> <c3 c2> <d3 d2> <g2 g1> <fs2 g2> [<g2 a2> <d2 ~>] [<c3 ~> <b2 ~>] <g2 <g3 g1>>")
  .sound("triangle")
  .slow(2)
  .lpf(perlin.range(120, 3000).slow(16))
  .gain(0.28)
  .every(6, p => p.palindrome().transpose("<0 12 -12 7 -5 0>").off(0.5, q => q.degradeBy(0.24)).sometimesBy(0.25, q => q.rev()).degradeBy(0.1))

$: note("<g4 b4> [~ d5] <d4 fs4> <c4 e4> <b3 d4> [g4 ~] <c4 e4> <a3 c4> <d4 fs4> [~ <a4 cs5>] [<b4 d5> ~] <c5 <b4 d5>>")
  .sound("piano")
  .slow(4)
  .lpf(perlin.range(900, 8600).slow(12))
  .gain(cosine.range(0.08, 0.16).slow(6))
  .every(4, p => p.palindrome().transpose("<7 0 -5 12 2 0>").sometimesBy(0.3, q => q.voicing().echo(3, 0.25, 0.14).degradeBy(0.16)).degradeBy(0.1))

$: note("<g4 a4> [d4 e4] <d4 fs4> <b3 c4> <d4 g4> <~ d4> [e4 fs4] <g4 <fs4 g4>> <a4 g4> <d5 b4> [~ <c5 a4>] <b4 d5>")
  .sound("supersaw")
  .slow(8)
  .lpf(perlin.range(320, 7200).slow(16))
  .gain(sine.range(0.06, 0.12).slow(10))
  .every(8, p => p.rev().echo(2, 0.25, 0.18).sometimesBy(0.38, q => q.transpose("<0 7 -5 12>").off(0.5, r => r.detune(9)).degradeBy(0.2)).sometimesBy(0.18, q => q.triode(0.22)).degradeBy(0.12))
...
$: arrange(
  ...
  .gain(0.32)
  .swing(0.26)
  .compressor("-18:2.8:6:0.02:0.22")
  .drive(0.18)
  .every(8, p => p.fast(2).off(0.25, q => q.degradeBy(0.6)).sometimesBy(0.3, q => q.rev()).degradeBy(0.22))
...
$: arrange(
  ...
  .hpf(560)
  .room(0.28)
  .every(4, p => p.rev().sometimesBy(0.26, q => q.speed(-1)).echo(2, 0.25, 0.16).degradeBy(0.22))
...
$: note("<d5 b4> <a4 g4> <e5 a4> [~ b4] <d5 fs5> <g5 e5> <fs5 ~> <e5 d5> [~ <a4 cs5>] <b4 d5> <c5 a4> [~ <g4 b4>]")
  ...
  .every(6, p => p.transpose("<7 12 0 -5 2 0>").palindrome().sometimesBy(0.34, q => q.slide(0.16).off(0.5, r => r.degradeBy(0.2)).degradeBy(0.14)).sometimesBy(0.22, q => q.ring(0.18)).degradeBy(0.12))