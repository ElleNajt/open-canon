...
$: note("<g3 g2> [gb3 f3] <e3 eb3> <b2 b1> <c3 c4> <d3 d4> <g2 g3> <fs2 g2> [<g2 a2> <d2 ~>] [<c3 ~> <b2 ~>] <g2 g3>")
  ...
  .gain(0.32)
  .every(6, p => p.palindrome().off(0.5, q => q.transpose(12).degradeBy(0.14)).degradeBy(0.06))

$: note("<g4 b4> [~ d5] <d4 fs4> <c4 e4> <b3 d4> [g4 ~] <c4 e4> <a3 c4> <d4 fs4> [~ <a4 cs5>] [<b4 d5> ~] <c5 <b4 d5>>")
  .sound("piano")
  .slow(4)
  .lpf(sine.range(900, 6400).slow(8))
  .gain(cosine.range(0.08, 0.2).slow(6))
  .every(4, p => p.palindrome().transpose(7).degradeBy(0.12))
...
$: arrange(
  ...
  [8, s("<come_closer closer_now we_are_listening breathe softly stay trust_the_loop hold_the_center ~>").chop(16).speed(rand.range(0.9, 1.12))],
  [8, s("<i_like_the_errors forgive_the_clicks repeat_with_me say_it_again ~ answer_me ~ make_room there_is_a_pattern this_is_mine_too>").chop(16).speed(rand.range(0.84, 1.2))],
