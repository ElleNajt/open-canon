$: note("<e2 [b1 ~] g1 [d2 e2] [g1 a1] [fs1 b1] ~ [e2 b1]>").sound("sawtooth")
  ...
  .attack(0.01).decay(0.6).sustain(0.25)
  .gain(0.4)
...
$: note("<[e4 g4] [~ b4] [d4 ~] [fs4 a4] [b3 e4] [~ g4] [a4 b4]>".add(perlin.range(-0.2,0.2)))
  .sound("sine").slow(7)
  .room(0.8).gain(0.18)

$: s("[~ hh:2] [hh:1 hh:3] ~ [hh:2 ~] [hh:1 hh:3] [~ hh:1]").fast(2)
  .gain(rand.range(0.05,0.14)).degradeBy(0.15)
  .pan(rand).hpf(6000)

$: s("bd ~ [~ bd] ~ [bd bd:1] [bd ~]").sometimes(x=>x.speed(0.85))
  .lpf(550).gain(0.38).shape(0.25)

$: note("<[b4 ~] [e5 g5] [~ d5] [fs5 ~] [e5 b4] [g5 ~] [~ a5]>").sound("pulse")
  .slow(11).lpf(sine.range(500,3200).slow(16))
  .gain(0.14).delay(0.5)