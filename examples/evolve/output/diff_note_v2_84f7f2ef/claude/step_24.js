$: note("<e2 [b1 ~] g1 [d2 e2] [g1 a1] [fs1 b1] ~ [e2 b1]>").sound("sawtooth")
  ...
  .attack(0.22).decay(3).sustain(0.005)
  .gain(0.58).shape(0.78)
...
$: s("[~ hh:2] [hh:1 ~] ~ [~ hh:3] [hh:1 hh:2] [~ hh:1]").fast(2)
  .gain(rand.range(0.03,0.11)).degradeBy(0.65)
  .pan(rand).hpf(6800)

$: s("bd ~ [~ bd] ~ [bd bd:1] [~ bd]").sometimes(x=>x.speed(0.55))
  .lpf(100).gain(0.52).shape(0.62).room(0.28)

$: note("<[b4 ~] [~ g5] [e5 d5] [~ fs5] [e5 ~] [g5 b4] [a5 ~] [~ d5]>").sound("pulse")
  .slow(16).lpf(sine.range(260,1900).slow(7))
  .gain(0.13).room(0.74).pan(perlin.range(0.15,0.85))