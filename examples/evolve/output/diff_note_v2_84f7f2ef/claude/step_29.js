$: note("<e2 [b1 ~] g1 [d2 e2] [g1 a1] [fs1 b1] ~ [e2 b1]>").sound("sawtooth")
  ...
  .attack(1.2).decay(4).sustain(0.02)
  .gain(0.48).shape(0.72)
...
$: note("<[e4 g4] [~ b4] [d4 ~] [fs4 a4] [b3 e4] [~ g4] [a4 b4] [d5 ~]>".add(perlin.range(-0.5,0.5)))
  .sound("sine").slow(16)
  .room(0.82).gain(0.11).lpf(cosine.range(320,2400).slow(11))

$: s("[~ hh:2] [hh:1 ~] ~ [~ hh:3] [hh:1 hh:2] [~ hh:1]").fast(2)
  .gain(rand.range(0.04,0.11)).degradeBy(0.58)
  .pan(rand).hpf(7200)

$: s("bd ~ [~ bd] ~ [bd bd:1] [~ bd]").rarely(x=>x.speed(0.7))
  .lpf(110).gain(0.5).shape(0.58).room(0.32)

$: note("<[b4 ~] [~ g5] [e5 d5] [~ fs5] [e5 ~] [g5 b4] [a5 ~] [~ d5]>").sound("pulse")
  .slow(14).lpf(sine.range(280,1600).slow(9))
  .gain(0.12).room(0.68).delay(0.28)