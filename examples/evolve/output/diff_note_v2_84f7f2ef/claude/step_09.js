$: note("<e2 [b1 ~] g1 [d2 e2] [g1 a1] [fs1 b1] ~ [e2 b1]>").sound("sawtooth")
  ...
  .attack(0.01).decay(0.4).sustain(0.25)
  .gain(0.44).shape(0.15)
...
$: note("<[e4 g4] [~ b4] [d4 ~] [fs4 a4] [b3 e4] [~ g4] [a4 b4] [d5 ~]>".add(perlin.range(-0.2,0.2)))
  .sound("sine").slow(9)
  .room(0.8).gain(0.18).pan(cosine.range(0.2,0.8).slow(11))

$: s("[~ hh:2] [hh:1 hh:3] ~ [hh:2 ~] [hh:1 hh:3] [~ hh:1]").fast(2)
  .gain(rand.range(0.05,0.11)).degradeBy(0.25)
  .pan(rand).hpf(6000)

$: s("bd ~ [~ bd] ~ [bd bd:1] [bd ~]").sometimes(x=>x.speed(0.85))
  .lpf(420).gain(0.38).shape(0.25)

$: note("<[b4 ~] [e5 g5] [~ d5] [fs5 ~] [e5 b4] [g5 ~] [~ a5] [b5 fs5]>").sound("pulse")
  .slow(11).lpf(sine.range(300,3200).slow(16))
  .gain(0.14).room(0.65)