...
).fast(2).swing(0.18).compressor("-18:4:6:0.01:0.2").every(4, p => p.degradeBy(0.06).sometimes(q => q.rev())).gain(0.86)
...
).fast(2).pan(perlin.range(0.15,0.85).slow(6)).hpf(4800).every(4, p => p.degradeBy(0.18).sometimes(q => q.rev())).gain(0.15)
...
).s("pulse").slow(2).lpf(sine.range(120, 2200).slow(10)).drive(0.24).gain(0.28)
...
$: arrange(
  ...
  [8, note("<c4 eb4 g4 bb4> <ab3 c4 eb4 g4> <f3 ab3 c4 eb4> <g3 bb3 d4 f4>")]
).s("supersaw").slow(4).lpf(sine.range(260, 4200).slow(16)).juxBy(0.45, p => p.detune(9).pan(0.78).hpf(180)).gain(0.12)
...
).slow(2).chop(16).delay(0.28).vowel("a e i o u").gain(0.11)

$: note("<c6 ~ [eb6 g6] ~ bb6> [~ g6] <eb6 ~> <g6 ~> <bb6 ~ [g6 eb6]> ~ <c7 ~ bb6 ~> <g6 ~> ~ <eb6 ~ [g6 bb6]>").s("sine").fast(4).lpf(perlin.range(900, 6800).slow(6)).tremolo(0.28).gain(0.065)