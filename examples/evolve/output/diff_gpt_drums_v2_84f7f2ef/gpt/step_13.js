...
$: arrange(
  [6, stack(s("bd ~ ~ ~ bd ~ ~ ~"), s("~ ~ ~ rim ~ ~ cr ~"))],
  [6, stack(s("bd ~ sn [~ cp] bd ~ sn ~"), s("~ ~ ~ ~ cr ~ ~ ~"))],
  [8, stack(s("bd [~ bd] sn [cp ~] bd [sn rim] ~ sn"), s("~ ~ cr ~ ~ ~ ~ ~"))],
  [8, stack(s("bd ~ sn cp bd [sn rim] cr [~ cp]"), s("~ ~ ~ ~ ~ ~ rim ~"))],
  [8, stack(s("bd*2 [sn rim] bd sn"), s("~ cr ~ ~ ~ ~ cr ~"))],
  [8, stack(s("bd [bd ~] sn [cp rim] bd [sn rim] cr [rim cp]"), s("~ ~ ~ ~ ~ rim ~ ~"))],
  [6, stack(s("bd ~ ~ ~ ~ ~ ~ ~"), s("~ ~ ~ ~ ~ ~ cr ~"))],
  [8, stack(s("bd [bd bd] sn [cp rim] bd [sn cp] cr sn"), s("~ ~ cr ~ ~ ~ cr ~"))]
).fast(2).swing(0.23).compressor("-22:6:6:0.008:0.15").every(4, p => p.degradeBy(0.12).sometimes(q => q.rev()).rarely(q => q.palindrome())).gain(0.84)

$: arrange(
  [8, s("hh*6 ~ hh*2")],
  [8, s("[hh*3 ~ hh oh]*2 [hh*2 ~ hh oh]*2")],
  [8, s("<hh hh hh oh>*8")],
  [8, s("~ [hh hh hh] ~ [hh oh hh]")],
  [8, s("oh ~ hh*3 ~ hh*3")],
  [8, s("[hh*4 ~] [hh*2 oh] [hh*3 ~ hh] [hh*2 oh]")],
  [8, s("<hh oh hh hh>*4")],
  [8, s("[hh*2 ~ hh] [oh ~ hh*2] [hh*3 ~] [~ hh oh]")]
).fast(2).pan(perlin.range(0.12, 0.88).slow(6)).djf(sine.range(0.7, 0.95).slow(12)).every(4, p => p.degradeBy(0.22).sometimes(q => q.rev()).rarely(q => q.ply(2))).gain(0.12)
...
$: arrange(
  ...
  [8, note("<c2 ~ eb2 g2> <~ g1 bb1 ~> <db2 ~ eb2 ~> <g1 ~ bb1 ~>")],
  [8, note("<c2 ~ eb2 ~> <g1 ~ bb1 ~> <ab1 ~ bb1 ~> <g1 ~ ~ ~>")]
).s("pulse").slow(2).lpf(sine.range(160, 2400).slow(10)).lpq(6).every(4, p => p.transpose(-12).rarely(q => q.rev())).gain(0.30)
...
).s("supersaw").slow(4).lpf(sine.range(320, 5200).slow(18)).delay(0.14).room(0.20).gain(0.12)
...
$: arrange(
  ...
  [8, s("okay ~ no_rush ~ more_space ~")],
  [8, s("again ~ follow_me still_with_me ~")],
  [8, s("shift_with_me ~ deeper_now ~ dont_let_go")],
  [8, s("one_more_time ~ keep_listening ~ lets_get_strange ~")]
).slow(2).chop(16).delay(0.30).room(0.16).gain(0.11)

$: note("<c6 ~ [eb6 g6] ~ bb6> [~ g6] <eb6 ~> <g6 ~> <bb6 ~ [g6 eb6]> ~ <c7 ~ bb6 ~> <fs6 ~> ~ <eb6 ~ [g6 bb6]>").s("sine").fast(4).lpf(perlin.range(900, 7600).slow(7)).phaser(0.18).echo(3, 0.25, 0.32).gain(0.065)