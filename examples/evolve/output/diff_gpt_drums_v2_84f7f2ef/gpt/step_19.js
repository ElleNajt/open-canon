...
$: arrange(
  ...
  [8,  stack(s("bd ~ sn ~ bd ~ sn ~"), s("~ ~ ~ ~ cr ~ ~ rim"))],
  ...
  [8,  stack(s("bd ~ ~ [~ bd] ~ ~ ~ ~"), s("~ ~ sn ~ ~ ~ sn ~"), s("~ ~ ~ ~ rim ~ ~ ~"))],
  [8,  stack(s("bd [bd ~] sn [cp rim] bd [sn cp] cr sn"), s("~ rd ~ ~ cr ~ rim ~"))]
).fast(2).swing(0.27).compressor("-22:6:6:0.008:0.15").every(4, p => p.degradeBy(0.16).sometimes(q => q.rev()).sometimesBy(0.33, q => q.ply(2)).rarely(q => q.echo(3, 0.125, 0.18))).gain(0.84)
...
).fast(2).pan(perlin.range(0.10, 0.90).slow(7)).phaser(sine.range(0.04, 0.22).slow(10)).every(4, p => p.degradeBy(0.22).sometimes(q => q.rev()).rarely(q => q.ply(2))).gain(0.12)
...
).s("pulse").slow(2).lpf(sine.range(110, 3000).slow(10)).every(4, p => p.sometimesBy(0.45, q => q.transpose(12)).sometimesBy(0.35, q => q.rev())).gain(0.26)
...
).s("supersaw").slow(4).lpf(perlin.range(650, 7600).slow(16)).every(8, p => p.rev().sometimesBy(0.5, q => q.palindrome())).gain(0.09)
...
).slow(2).chop(16).delay(0.34).djf(sine.range(0.18, 0.92).slow(18)).gain(0.10)

$: note("<c6 ~ [eb6 g6] ~ bb6> [~ g6] <eb6 ~> <g6 ~> <bb6 ~ [g6 eb6]> <db6 ~> <c7 ~ bb6 ~> <~ ~> <fs6 ~> <eb6 ~ [g6 bb6]>")
  .s("triangle")
  .fast(4)
  .echo(4, 0.25, 0.30)
  .every(6, p => p.rev().sometimesBy(0.6, q => q.palindrome()))
  .gain(0.06)