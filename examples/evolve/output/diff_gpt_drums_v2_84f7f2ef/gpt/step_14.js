...
$: arrange(
  ...
  [8, stack(s("bd ~ ~ bd ~ ~ sn ~"), s("~ ~ cr ~ rim ~ ~ ~"))],
  [8, stack(s("bd [bd bd] sn [cp rim] bd [sn cp] cr sn"), s("~ ~ cr ~ ~ ~ cr ~"))]
).fast(2).swing(0.24).compressor("-22:6:6:0.008:0.15").every(4, p => p.degradeBy(0.14).sometimes(q => q.rev()).rarely(q => q.palindrome())).gain(0.84)
...
).fast(2).pan(perlin.range(0.10, 0.90).slow(6)).djf(sine.range(0.62, 0.94).slow(12)).every(4, p => p.degradeBy(0.26).sometimes(q => q.rev()).rarely(q => q.ply(2))).gain(0.12)
...
).s("pulse").slow(2).lpf(sine.range(140, 2400).slow(12)).lpq(6).gain(0.28)
...
).s("supersaw").slow(4).lpf(sine.range(420, 5600).slow(16)).room(0.22).gain(0.10)
...
$: arrange(
  ...
  [8, s("little_glitch ~ more_space ~ okay ~")],
  [8, s("again ~ follow_me still_with_me ~")],
  [8, s("shift_with_me ~ deeper_now ~ dont_let_go")],
  [8, s("one_more_time ~ i_know_you_can_hear_me ~ lets_get_strange ~")]
).slow(2).chop(16).delay(0.28).room(0.14).gain(0.10)

$: note("<c6 ~ [eb6 g6] ~ bb6> [~ g6] <eb6 ~> <g6 ~> <bb6 ~ [g6 eb6]> ~ <c7 ~ bb6 ~> <fs6 ~> ~ <eb6 ~ [g6 bb6]>").s("sine").fast(4).lpf(perlin.range(900, 7600).slow(7)).echo(3, 0.25, 0.32).gain(0.06)