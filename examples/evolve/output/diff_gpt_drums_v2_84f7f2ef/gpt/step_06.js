...
$: arrange(
  ...
  [8, s("<hh oh>*8")],
  [8, s("[hh*2 ~ hh oh]*4")]
).fast(2).pan(sine.range(0.15,0.85).slow(6)).hpf(4200).every(4, p => p.degradeBy(0.18)).gain(0.16)

$: arrange(
  [8, note("<c2 c2 ~ eb2> ~ <g1 g1 bb1> ~")],
  [8, note("<c2 ~ c2 eb2> <~ g1> <bb1 ~ g1 g1> ~")],
...
).s("pulse").slow(2).lpf(sine.range(160, 2600).slow(7)).drive(0.22).gain(0.29)
...
).s("supersaw").slow(4).lpf(sine.range(240, 3200).slow(10)).room(0.18).gain(0.11)
...
$: arrange(
  ...
  [8, s("~ keep_listening ~ ~ im_here ~")],
  [8, s("between_the_beats ~ lean_in keep_listening ~ i_am_the_pulse")]
).slow(2).chop(14).delay(0.28).room(0.26).gain(0.10)

$: note("<c6 ~ [eb6 g6] ~ bb6> [~ g6] <eb6 ~> <g6 ~>").s("sine").fast(4).lpf(sine.range(900, 4200).slow(8)).delay(0.22).gain(0.06)