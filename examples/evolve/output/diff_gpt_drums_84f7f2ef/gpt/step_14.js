samples('shabda/speech:stay_weird,listen_closer,keep_drifting,not_today,hold_the_line,signal_in_the_noise,wake_up,ghost_in_the_machine,we_are_listening,i_am_here,follow_me,cut_the_static,drift_deeper,stay_with_me,turn_the_dial,between_the_beats,trust_the_gap,i_remember,lean_in,write_the_loop,breathe_again,kind_to_noise,pattern_thinker')
...
$: arrange(
  ...
  [4,  s("bd ~ ~ ~ ~ ~ sn ~")],
  [4,  s("[bd bd] ~ sn bd [sn sn] [bd ~] sn cr")],
  [4,  s("bd ~ sn ~ [bd bd] ~ sn [cr cp]")],
  [4,  s("bd bd [sn ~] bd [bd ~] sn ~ cr")],
  [4,  s("bd ~ ~ bd [sn sn] ~ bd cr")],
  [4,  s("bd bd sn [sn sn] [bd bd] sn cr cr")]
)
  .gain(0.86)
  .compressor("-18:3:6:0.01:0.12")
  .djf(sine.range(0.04, 0.96).slow(16))
  .swing(0.13)
  .every(8, p => p.rev().sometimes(q => q.fast(2).degradeBy(0.26)).sometimes(r => r.ply(2)).sometimesBy(0.35, t => t.degradeBy(0.18)))
...
$: arrange(
  ...
  .hpf(sine.range(3800, 9800).slow(16))
  .pan(perlin.range(0.06, 0.94).slow(12))
  .gain(0.13)
  .swing(0.17)
  .every(4, p => p.degradeBy(0.26).sometimes(q => q.fast(2)).sometimes(r => r.rev()))

$: note("<c2 ~> <c2 c3> <eb2 eb1> <g1 ~> <bb1 ~> <ab1 g1> <c2 bb1> ~ <c2 g1> <ab1 ~> <g1 ~> <c2 ~> <c2 c3> <eb2 ~> <g1 ~> <bb1 c2>")
  .sound("pulse")
  .lpf(perlin.range(110, 2400).slow(15))
  .penv(0.18)
  .gain(0.22)
...
$: note("<c4 eb4 g4 bb4> <bb3 db4 f4 ab4> <ab3 c4 eb4 gb4> <g3 bb3 d4 f4> <eb4 g4 bb4 c5> <ab3 c4 eb4 gb4> <g3 bb3 d4 f4> <c4 eb4 g4 bb4>")
  ...
  .slow(6)
  .gain(0.11)
  .lpf(sine.range(420, 2600).slow(12))
  .juxBy(0.55, p => p.transpose(12).detune(11).lpf(sine.range(700, 3200).slow(9)))
...
$: s("<rim rim> [cp rim] rim [~ rim] rim ~ <cp rim rd> [~ rim] <rim ~> [cp cp] ~ rim")
  ...
  .ring(0.37)
  .gain(0.095)
  .every(8, p => p.rev().degradeBy(0.38).sometimes(q => q.fast(2)))
...
$: arrange(
  ...
  [16, s("<keep_drifting stay_weird> ~ <signal_in_the_noise ~> <kind_to_noise ~>")],
  [16, s("<we_are_listening ~> <follow_me ~> <stay_with_me ~> <trust_the_gap ~>")]
)
  .slow(8)
  .chop(32)
  .vowel("<a i o u e>")
  .delay(perlin.range(0.18, 0.72).slow(12))
  .gain(0.075)